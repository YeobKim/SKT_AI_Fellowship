...
class WCAB(nn.Module):
    def __init__(self, features, kernel_size, reduction, bias, act):
        ...
        self.conv2 = nn.Conv2d(in_channels=features, out_channels=features, kernel_size=3, padding=2, dilation=2,
                               bias=False)
        self.conv3 = nn.Conv2d(in_channels=features, out_channels=features, kernel_size=3, padding=3, dilation=3,
                               bias=False)
    ...
    def forward(self, x):
        ...
        res = self.body(flatdata)
        res = self.CA(res)
        res += x
        return res

##########################################################################
## Wide Receptive Field Original Resolution Block (ORB)
class WORB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, num_cab):
        super(WORB, self).__init__()
        modules_body = []
        modules_body = [WCAB(n_feat, kernel_size, reduction, bias=bias, act=act) for _ in range(num_cab)]
        modules_body.append(conv(n_feat, n_feat, kernel_size))
        self.body = nn.Sequential(*modules_body)
...
##########################################################################
class WORSNet(nn.Module):
    def __init__(self, n_feat, scale_orsnetfeats, kernel_size, reduction, act, bias, scale_unetfeats, num_cab):
        super(WORSNet, self).__init__()

        self.worb1 = WORB(n_feat + scale_orsnetfeats, kernel_size, reduction, act, bias, num_cab)
        self.worb2 = WORB(n_feat + scale_orsnetfeats, kernel_size, reduction, act, bias, num_cab)
        self.worb3 = WORB(n_feat + scale_orsnetfeats, kernel_size, reduction, act, bias, num_cab)
...
class SKSAK(nn.Module):
    def __init__(self, channels=3, features=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3,
        ...
        self.edgemodule = EdgeMoudule(channels, features)
        ...
        self.stage3_worsnet = WORSNet(features, scale_orsnetfeats, kernel_size, reduction, act, bias, scale_unetfeats,
                                    num_cab)
        ...
        # self.concat23 = conv(features * 2, features + scale_orsnetfeats, kernel_size, bias=bias)
        self.concat23 = conv(features, features + scale_orsnetfeats, kernel_size, bias=bias)
        self.tail = conv(features + scale_orsnetfeats, channels, kernel_size, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        # Edge Module
        edge = x3_img - self.edgemodule(x3_img)
        ...
        ##-------------------------------------------
        ## Compute Shallow Features
        x3_img_feat = self.ch2feat(x3_img)
        edge_feat = self.ch2feat(edge)
        x3_img_edge = self.concat12(torch.cat([x3_img_feat, edge_feat], 1))
        x3 = self.shallow_feat3(x3_img_edge)

        ## Concatenate SAM features of Stage 2 with shallow features of Stage 3
        # x3_cat = self.concat23(torch.cat([x3, x3_samfeats], 1))
        x3_cat = x3 + x3_samfeats
        x3_cat = self.concat23(x3_cat)

        x3_cat = self.stage3_worsnet(x3_cat, feat2, res2)

        out = self.tail(x3_cat) + x3_img

        return out, edge, stage2_img, stage1_img