...
parser.add_argument("--resume_epochs", type=int, default=23, help="Number of training epochs When training resume")
...
def main():
    ...
    # Scheduler
    scheduler_cos = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=60, eta_min=opt.last_lr)
    ...
    load = torch.load(os.path.join(opt.logdir, 'SKT_AI_blur4_23_0.0308.pth'))
    ...
    for epoch in range(opt.resume_epochs, opt.epochs):
        ...
        for i, (degraded_train, gt_train) in enumerate(dataset_train, 0):
            ...
            blurnum = random.randint(0, 1)
            randomnum = random.randint(0, 2)

            # Make Edge Image.
            original_edge = torch.FloatTensor(degraded_train.shape[0], 3, opt.patchsize, opt.patchsize)
            degraded_blur = torch.FloatTensor(degraded_train.shape[0], 3, opt.patchsize, opt.patchsize)

            for j in range(degraded_train.shape[0]):
                original_edge[j] = toTensor((toPILImg(gt_train[j]).filter(ImageFilter.FIND_EDGES)))
                degraded_blur[j] = toTensor((toPILImg(gt_train[j]).filter(ImageFilter.GaussianBlur(radius=4))))

            if blurnum == 0:
                degraded_img = degraded_blur
            else:
                degraded_img = degraded_train

            if randomnum == 0:
                noise = torch.FloatTensor(degraded_train.shape[0], 1, opt.patchsize, opt.patchsize).normal_(mean=0, std=5 / 255.)
                degraded_img = degraded_img + noise
            elif randomnum == 1:
                noise = torch.FloatTensor(degraded_train.shape[0], 1, opt.patchsize, opt.patchsize).normal_(mean=0, std=10 / 255.)
                degraded_img = degraded_img + noise
            else:
                # noise = torch.FloatTensor(degraded_train.size()).normal_(mean=0, std=15 / 255.)
                noise = torch.FloatTensor(degraded_train.shape[0], 1, opt.patchsize, opt.patchsize).normal_(mean=0, std=15 / 255.)
                degraded_img = degraded_img + noise
